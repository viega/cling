...
uint8_t test_N[] = {'\x17'};
uint8_t N[]
    = {'\x92', '\x0A', '\xD1', '\xED', '\xED', '\xC7', '\x17', '\xBA', '\x3B',
       '\xFE', '\x24', '\xBB', '\xAB', '\xE1', '\x08', '\x37', '\xF3', '\xD7',
       '\xD2', '\x16', '\x7C', '\xAD', '\x8B', '\x1C', '\x31', '\xFC', '\x39',
       '\x87', '\x81', '\xD9', '\x39', '\xDB', '\x51', '\x8A', '\xDE', '\xC7',
       '\x33', '\xC6', '\xA5', '\x97', '\x94', '\x70', '\xA2', '\xF7', '\x91',
       '\x13', '\x19', '\x0C', '\x31', '\xC5', '\x9F', '\x1D', '\xF5', '\x0F',
       '\x7D', '\x59', '\x23', '\xB7', '\x30', '\x01', '\x95', '\x93', '\x28',
       '\x66', '\x0C', '\x90', '\xE1', '\xD7', '\xB0', '\x26', '\x6F', '\xC3',
       '\x8A', '\x08', '\xA2', '\x93', '\x78', '\xEC', '\xBA', '\x2A', '\xEB',
       '\x2B', '\x02', '\x72', '\xF4', '\x4B', '\x0F', '\x79', '\x34', '\x7B',
       '\x9B', '\xFD', '\x0E', '\x87', '\x3E', '\x4D', '\x2D', '\x69', '\xA1',
       '\xE5', '\xFF', '\x1B', '\x67', '\x1A', '\xA4', '\x59', '\xB7', '\x1D',
       '\x38', '\x61', '\xE6', '\x3C', '\xAB', '\x8A', '\x97', '\xC6', '\xEE',
       '\xC8', '\x24', '\xDE', '\x66', '\x9F', '\x64', '\x7B', '\x1F', '\xB7',
       '\x31', '\xFC', '\x08', '\x53', '\xDA', '\xD2', '\xD0', '\xF8', '\x44',
       '\x58', '\x80', '\x88', '\x81', '\x0A', '\x2E', '\x19', '\xC8', '\x72',
       '\x29', '\x1E', '\x23', '\xC5', '\xD3', '\x18', '\x97', '\x9E', '\x39',
       '\xE0', '\x01', '\x61', '\x68', '\x00', '\xB7', '\x42', '\x44', '\x6E',
       '\xEA', '\xCB', '\xAF', '\x3E', '\xE1', '\xC8', '\xA2', '\x06', '\x7A',
       '\xC2', '\x3F', '\x3B', '\xC2', '\xFB', '\x1D', '\x35', '\x05', '\x17',
       '\xE0', '\x28', '\x55', '\x0E', '\xEC', '\x60', '\x13', '\x78', '\x71',
       '\xA8', '\x03', '\xF5', '\x9B', '\x97', '\x06', '\x10', '\xEE', '\x3E',
       '\xCC', '\x26', '\x26', '\x9D', '\x6C', '\x0C', '\x64', '\x7F', '\xBC',
       '\x5B', '\xDB', '\x5F', '\xF1', '\x55', '\x85', '\xD5', '\x4F', '\x45',
       '\xD7', '\x56', '\x4D', '\x14', '\xA7', '\x75', '\x9E', '\x31', '\xD0',
       '\xB9', '\x1B', '\x1A', '\x2C', '\x12', '\xF1', '\x73', '\x22', '\x1B',
       '\xE2', '\xAE', '\x0B', '\x33', '\xBA', '\x9A', '\xD5', '\x09', '\xF4',
       '\xE3', '\x88', '\x8C', '\x4F', '\x8E', '\x7C', '\x75', '\x67', '\xB4',
       '\x87', '\xC3', '\xA0', '\x6B'};
...
build_hex_dump(unsigned char *bytes, uint32_t len)
{
    ...
    // Estimate that each byte takes 10 chars to represent for now.  Come back
    // and do the math.
...
print_bignum(mp_int *n)
{
    ...
    if (mp_radix_size(n, 10, &size) != MP_OKAY) {
        printf("1\n");
        exit(-1);
    }
    s = (char *)malloc(size);

    if (mp_to_radix(n, s, size, &written, 10) != MP_OKAY) {
        printf("2\n");
        exit(-1);
    }
...
srp_compute_k(srp_ctx *ctx)
{
    ...
    if (mp_from_ubin(&mp_k, (const uint8_t *)&tag.bytes, SHA512_TAG_LENGTH)
        != MP_OKAY) {
        printf("3\n");
        exit(-1);
    }
    if (mp_mod(&mp_k, &mp_N, &mp_k) != MP_OKAY) {
        printf("4\n");
        exit(-1);
    }
}
...
srp_init(srp_ctx *ctx)
{
    ...
    if (!inited) {
        if (mp_init(&mp_N) != MP_OKAY) {
            printf("5\n");
            exit(-1);
        }
        if (mp_init(&mp_nil) != MP_OKAY) {
            printf("6\n");
            exit(-1);
        }
        if (mp_init(&mp_g) != MP_OKAY) {
            printf("7\n");
            exit(-1);
        }
        if (mp_from_ubin(&mp_N, N, sizeof(N)) != MP_OKAY) {
            printf("8\n");
            exit(-1);
        }
        mp_set(&mp_nil, 0);
    ...
    ctx->k_ptr    = &mp_k;
    ctx->Zero_ptr = &mp_nil;
    if (mp_init(&ctx->u) != MP_OKAY) {
        exit(-1);
    }
    if (mp_init(&ctx->a) != MP_OKAY) {
        exit(-1);
    }
    if (mp_init(&ctx->b) != MP_OKAY) {
        exit(-1);
    }
    if (mp_init(&ctx->A) != MP_OKAY) {
        exit(-1);
    ...
    if (mp_init(&ctx->x) != MP_OKAY) {
        exit(-1);
    }
    if (mp_init(&ctx->v) != MP_OKAY) {
        exit(-1);
    }
    if (mp_init(&ctx->S) != MP_OKAY) {
        exit(-1);
    }
    ctx->salt     = NULL;
    ctx->username = NULL;
...
srp_compute_x(srp_ctx *ctx, uint8_t *pw, uint32_t pwlen)
{
    ...
    sha512_final(&ctx->hctx, &tag, &error);
    if (mp_from_ubin(&ctx->x, (const uint8_t *)&tag.bytes, SHA512_TAG_LENGTH)
        != MP_OKAY) {
        printf("11\n");
        exit(-1);
    }
    if (mp_mod(&ctx->x, ctx->N_ptr, &ctx->x) != MP_OKAY) {
        printf("12\n");
        exit(-1);
    }
}
...
srp_compute_v(srp_ctx *ctx, uint8_t *pw, uint32_t pwlen)
{
    ...
    // printf("x(2): ");
    // print_bignum(&ctx->x);
    if (mp_exptmod(ctx->g_ptr, &ctx->x, &mp_N, &ctx->v) != MP_OKAY) {
        printf("13\n");
        exit(-1);
    }
    // printf("v(2): ");
...
srp_compute_A(srp_ctx *ctx)
{
    ...
    if (mp_mod(&ctx->a, ctx->N_ptr, &ctx->a) != MP_OKAY) {
        printf("16\n");
        exit(-1);
    }
    // A = g^a mod N
    if (mp_exptmod(ctx->g_ptr, &ctx->a, ctx->N_ptr, &ctx->A) != MP_OKAY) {
        printf("17\n");
        exit(-1);
    }
    // printf("g(1): ");
...
srp_compute_B(srp_ctx *ctx)
{
    ...
    if (mp_init(&expr1) != MP_OKAY) {
        printf("18\n");
        exit(-1);
    }
    if (mp_init(&expr2) != MP_OKAY) {
        printf("19\n");
        exit(-1);
    }

    srp_select_random(ctx, &ctx->b);
    if (mp_mod(&ctx->b, ctx->N_ptr, &ctx->b) != MP_OKAY) {
        printf("20\n");
        exit(-1);
    }
...
{
    sha512_tag tag;
    uint8_t    buf[2 * SRP_PRIME_SIZE / 8] = {
        0,
    };
    size_t outlen;
    int    error;
    ...
    if ((code = mp_to_ubin(&ctx->A, buf, SRP_PRIME_SIZE / 8, &outlen))
        != MP_OKAY) {
        printf("24\n");
    ...
    if (mp_from_ubin(&ctx->u, (const uint8_t *)&tag.bytes, SHA512_TAG_LENGTH)
        != MP_OKAY) {
        printf("26\n");
        exit(-1);
    }
    if (mp_mod(&ctx->u, ctx->N_ptr, &ctx->u) != MP_OKAY) {
        printf("27\n");
        exit(-1);
    }
}
...
srp_party_1_compute_S(srp_ctx *ctx)
{
    ...
    if (mp_init(&expr3) != MP_OKAY) {
        exit(-1);
    }
    if (mp_init(&expr4) != MP_OKAY) {
        exit(-1);
    }
    if (mp_init(&expr5) != MP_OKAY) {
        exit(-1);
    }

    if (mp_exptmod(ctx->g_ptr, &ctx->x, ctx->N_ptr, &expr1) != MP_OKAY) {
        exit(-1);
    }
    if (mp_mul(ctx->k_ptr, &expr1, &expr2) != MP_OKAY) {
        exit(-1);
    }
    if (mp_sub(&ctx->B, &expr2, &expr3) != MP_OKAY) {
        exit(-1);
    }
    if (mp_mul(&ctx->u, &ctx->x, &expr4) != MP_OKAY) {
        exit(-1);
    }
    if (mp_add(&ctx->a, &expr4, &expr5) != MP_OKAY) {
        exit(-1);
    }
    if (mp_exptmod(&expr3, &expr5, ctx->N_ptr, &ctx->S) != MP_OKAY) {
        exit(-1);
    }
    printf("Yes.\n");
...
srp_party_2_compute_S(srp_ctx *ctx)
{
    ...
    if (mp_exptmod(&ctx->v, &ctx->u, ctx->N_ptr, &expr1) != MP_OKAY) {
        exit(-1);
    }
    if (mp_mulmod(&expr1, &ctx->A, ctx->N_ptr, &expr2) != MP_OKAY) {
        exit(-1);
    }
    if (mp_exptmod(&expr2, &ctx->b, ctx->N_ptr, &ctx->S) != MP_OKAY) {
        exit(-1);
    }
    printf("Yes.\n");

    // Old and busted
    // if (mp_mulmod(&ctx->A, &ctx->v, ctx->N_ptr, &expr1) != MP_OKAY) {
    // exit(-1); }
    //   if (mp_exptmod(&expr1, &ctx->u, ctx->N_ptr, &expr2) != MP_OKAY) {
    //   exit(-1); }
    // if (mp_exptmod(&expr2, &ctx->b, ctx->N_ptr, &ctx->S) != MP_OKAY) {
    // exit(-1); }
...
srp_check_party_1_params(srp_ctx *ctx)
{
    ...
    if (mp_mod(&ctx->A, ctx->N_ptr, &tmp) != MP_OKAY) {
        printf("b\n");
        exit(-1);
    }
...
srp_check_party_2_params(srp_ctx *ctx)
{
    ...
    if (mp_mod(&ctx->B, ctx->N_ptr, &tmp) != MP_OKAY) {
        printf("d\n");
        exit(-1);
    }
    if (mp_cmp_mag(&tmp, ctx->Zero_ptr) == MP_EQ) {
...
compute_key_material(srp_ctx *ctx)
{
    ...
    if (mp_to_ubin(&ctx->S, buf, buflen, &outlen) != MP_OKAY) {
        printf("e\n");
        exit(-1);
    }
...
compute_proofs(srp_ctx *ctx)
{
    ...
    sha512_update(&ctx->hctx,
                  ctx->keymatter.bytes,
                  sizeof(ctx->keymatter),
                  &error);
...
srp_party_2_step_1(srp_ctx *ctx)
{
    ...
    print_bignum(&ctx->S);
    printf("\n");
    if (!srp_check_party_1_params(ctx)) {
        printf("Fuck.\n");
        return;
    }
...
srp_party_1_step_2(srp_ctx *ctx, uint8_t *pw, uint32_t pwlen)
{
    ...
    // print_bignum(&ctx->S);
    // printf("\n");
    if (!srp_check_party_2_params(ctx)) {
        return;
    }
    compute_key_material(ctx);
    compute_proofs(ctx);
